...
const prayers = JSON.parse(readFileSync(new URL('../app/content/prayers.json', import.meta.url)));
...
test('Higginson retains his historical wording instead of a modern substitution', () => {
  const p = prayers.find(p => p.id === 'epictetus-prayer-of-self-examination-in-illness-and-death');
  assert.ok(p.editions.some(e => e.text.includes('the senses, the instincts')));
});
